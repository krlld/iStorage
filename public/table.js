    ...
    $.ajax({
      ...
      success: (sum) => {
        if (
          ...
          alert('На складе отсутствует заданное количество товара');
        ...
        $.ajax({
          ...
          success: (data) => {
            if (document.activeElement.getAttribute('formaction') === '/inventory/plus') {
              document.getElementById(`count${data.id}`).innerText =
                Number(sum.sum) + Number(data.count);
            }
            if (document.activeElement.getAttribute('formaction') === '/inventory/minus') {
              document.getElementById(`count${data.id}`).innerText =
                Number(sum.sum) - Number(data.count);
            }
